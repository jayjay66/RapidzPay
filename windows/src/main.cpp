...
// Copyright (c) 2009-2012 The rapidz developers
...
unsigned char pchMessageStart[4] = { 0xcd, 0xca, 0xcf, 0x8d };
unsigned int nStakeMinAge = 60 * 60 * 24*15;	
unsigned int nStakeMaxAge = 60 * 60 * 24*15;
unsigned int nStakeTargetSpacing = 60;			

int64 nChainStartTime = 1509586992;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if(nHeight==1)
       nSubsidy = 10000000 * COIN; //Premin: 10M
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
   int64 nSubsidy = 0;
        ...
        nSubsidy = (nCoinAge * 0.1 * COIN )/ 365;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal rapidz miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in rapidz
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "2017/11/2,RapidZ is upcoming release wish is a great coin!";
        CTransaction txNew;
      ...
      if (true  && (block.GetHash() != hashGenesisBlock))
        ...
        assert(block.hashMerkleRoot == uint256("0x4481fe0bc448b0290d8a3ff6d6bb9e30e47d45ce45913a80af2de5847cb4598e"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("rapidzMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("rapidzMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("rapidzMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadrapidzMiner(void* parg);

static bool fGeneraterapidzs = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void rapidzMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("rapidz-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneraterapidzs || fProofOfStake)
    {
        if (fShutdown)
            return;
        ...
        while (vNodes.empty() || IsInitialBlockDownload() || pwallet->IsLocked())
        {
            ...
            if (fShutdown)
                return;
            if (!fGeneraterapidzs && !fProofOfStake)
                return;
        }
        ...
        printf("Running rapidzMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadrapidzMiner(void* parg)
{
    ...
    try
    {
        ...
        rapidzMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadrapidzMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadrapidzMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadrapidzMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generaterapidzs(bool fGenerate, CWallet* pwallet)
{
    fGeneraterapidzs = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneraterapidzs = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d rapidzMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadrapidzMiner, pwallet))
                printf("Error: NewThread(ThreadrapidzMiner) failed\n");
            Sleep(10);
        }
    }
}
